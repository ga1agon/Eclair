...
namespace GENESIS.GPU.Vulkan {
	...
	public unsafe class VulkanPlatform : IPlatform {
		...
		public VulkanPlatform(PlatformOptions? options = null) {
		...
		public IReadOnlyCollection<VulkanDevice> GetDevices() {
			var nativeDevices = API.GetPhysicalDevices(VK);
			var devices = new List<VulkanDevice>();

			foreach(var nativeDevice in nativeDevices) {
				devices.Add(new VulkanDevice(this, nativeDevice));
			}
			
			return devices;
		}

		public VulkanDevice CreateDevice(int index) {
			var nativeDevices = API.GetPhysicalDevices(VK);
			...
			return new VulkanDevice(this, nativeDevices.ElementAt(index));
		}
